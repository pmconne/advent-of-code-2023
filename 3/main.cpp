...
bool containsSymbol(const char* line, int start, int end) {
...
int part1(const char** input, int width, int height) {
  cout << width << "x" << height << endl;
  int sum = 0;
  for (int y = 0; y < height; y++) {
    auto curLine = input[y];
    cout << curLine << endl;
    for (int x = 0; x < width; x++) {
      auto curChar = curLine[x];
      if (isdigit(curChar)) {
        int start = x;
        int end = x;
        while (isdigit(curLine[end]))
          end++;
        ...
        //cout << "  len " << len;
        for (int i = 0; i < len; i++) {
          // cout << "  " << curLine[start + i];
          int digit = curLine[start + i] - '0';
          // cout << " > " << digit;
          int mult = len - i - 1;
          // cout << " > " << mult;
          int term = digit * tenToPower(mult);
          // cout << " > " << term << endl;
          partNo += term;
        }

        int left = start > 0 ? start - 1 : 0;
        bool isPart = isSymbol(curLine[left]) || isSymbol(curLine[end]);
        if (!isPart) {
          if (y > 0 && containsSymbol(input[y - 1], left, end))
            isPart = true;
          else if (y + 1 < height && containsSymbol(input[y + 1], left, end))
            isPart = true;
        }

        cout << "  " << partNo << " " << isPart << endl;
        if (isPart)
          sum += partNo;
...
int main(int argc, const char** argv) {
  ...
  // cout << part1(sampleInput, strlen(sampleInput[0]), sizeof(sampleInput)/sizeof(char*)) << endl;
  cout << part1(realInput, strlen(realInput[0]), sizeof(realInput)/sizeof(char*)) << endl;
  return 0;
}
