...
function part2(input) {
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < width; i++) {
      ...
      const partNo = Number.parseInt(str, 10);
      //console.log(`parsed ${str} as ${partNo}`);
      const partIdx = partNos.length;
      partNos.push(partNo);
      row[i] = partIdx;
      if (partNo > 10) {
        row[++i] = partIdx;
  ...
  console.log(`Part numbers: ${JSON.stringify(partNos)}`);
  let sum = 0;
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let r = y - 1; r <= y + 1; r++) {
        for (let c = x -1; c <= x + 1; c++) {
          const row = grid[r];
          const partIdx = row ? row[c] : undefined;
          if (undefined !== partIdx)
            adj.add(partIdx);
        }
      }

      if (adj.size === 2) {
        const pair = Array.from(adj);
        const part1 = partNos[pair[0]];
        const part2 = partNos[pair[1]];
        const ratio = part1 * part2;
        //console.log(`${part1}x${part2}=${ratio}`);
        sum += ratio;
      }
    }
  }
