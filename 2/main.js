...
function part1(input) {
  ...
  for (const line of lines) {
    ...
    const rest = parts[1];
    parts = parts[0].split(" ");
    const gameId = Number.parseInt(parts[1], 10);

    const counts = { red: 0, green: 0, blue: 0 };
    const games = rest.trim().split(";");
    for (const game of games) {
      parts = game.trim().split(",");
      for (const part of parts) {
        const pair = part.trim().split(" ");
        const color = pair[1].trim();
        const count = Number.parseInt(pair[0].trim());
        counts[color] = Math.max(counts[color], count);
      }
    }

    if (counts.red > maximums.red || counts.green > maximums.green || counts.blue > maximums.blue) {
      console.log(`${line} is impossible ${JSON.stringify(counts)}`);
    } else {
      console.log(`${line} is possible ${JSON.stringify(counts)}`);
      sum += gameId;
    }
  }
...
function part2(input) {
  ...
  for (const line of lines) {
    ...
    const maximums = { red: 0, green: 0, blue: 0 };
    const games = rest.trim().split(";");
    for (const game of games) {
      parts = game.trim().split(",");
      for (const part of parts) {
        ...
        const color = pair[1].trim();
        const count = Number.parseInt(pair[0].trim());
        maximums[color] = Math.max(maximums[color], count);
      }
    }

    const power = maximums.red * maximums.green * maximums.blue;
    console.log(`${power} ${gameId}`);
    sum += power;
  }
